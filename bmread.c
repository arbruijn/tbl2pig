...
#define REMOVE_EOL(s)		do { remove_char((s),'\n');remove_char((s),'\r'); } while (0)
...
bitmap_index bm_load_sub( char * filename )
{
	...
	mprintf(( 0, "NOT FOUND bitmap '%s' in pig!\n", fname ));
...
int bm_read_tbl(FILE *InfoFile)
{
	...
	while (fgets(inputline, LINEBUF_SIZE, InfoFile)) {
		int l;
		char *temp_ptr;
		...
		linenum++;
			...
			while ((l=strlen(inputline)) && inputline[l-1]=='\\') {
				if (!fgets(inputline+l-1,LINEBUF_SIZE-(l-1), InfoFile))
					break;
				REMOVE_EOL(inputline);
				linenum++;
			}
...
//--01/19/95, mk--				Weapon_info[n].damage_force = fl2f(atof(equal_ptr));
	...
	if ((n != 14) && (Weapon_info[n].ammo_usage == 0) && (Weapon_info[n].energy_usage == 0))
...
#define ELMS(xs) ((sizeof(xs))/sizeof((xs)[0]))
...
void bm_read_all(FILE * fp)
{
	...
	Assert( fread( &NumTextures, sizeof(int), 1, fp ) );
	Assert( NumTextures > 0 && NumTextures <= ELMS(Textures) );
	Assert( fread( Textures, sizeof(bitmap_index), MAX_TEXTURES, fp ) );
	Assert( fread( TmapInfo, sizeof(tmap_info), MAX_TEXTURES, fp ) );
	
	Assert( fread( Sounds, sizeof(ubyte), MAX_SOUNDS, fp ) );
	Assert( fread( AltSounds, sizeof(ubyte), MAX_SOUNDS, fp ) );

	Assert( fread( &Num_vclips, sizeof(int), 1, fp ) );
	Assert( Num_vclips <= ELMS(Vclip) ); // Num_vclips seems unused, it's 0
	Assert( fread( Vclip, sizeof(vclip), VCLIP_MAXNUM, fp ) );

	Assert( fread( &Num_effects, sizeof(int), 1, fp ) );
	Assert( Num_effects > 0 && Num_effects <= ELMS(Effects) );
	Assert( fread( Effects, sizeof(eclip), MAX_EFFECTS, fp ) );

	Assert( fread( &Num_wall_anims, sizeof(int), 1, fp ) );
	Assert( Num_wall_anims > 0 && Num_wall_anims <= ELMS(WallAnims) );
	Assert( fread( WallAnims, sizeof(wclip), MAX_WALL_ANIMS, fp ) );

	Assert( fread( &N_robot_types, sizeof(int), 1, fp ) );
	Assert( N_robot_types > 0 && N_robot_types <= MAX_ROBOT_TYPES );
	Assert( fread( Robot_info, sizeof(robot_info), MAX_ROBOT_TYPES, fp ) );

	Assert( fread( &N_robot_joints, sizeof(int), 1, fp ) );
	Assert( N_robot_joints > 0 && N_robot_joints <= MAX_ROBOT_JOINTS );
	Assert( fread( Robot_joints, sizeof(jointpos), MAX_ROBOT_JOINTS, fp ) );

	Assert( fread( &N_weapon_types, sizeof(int), 1, fp ) );
	Assert( N_weapon_types > 0 && N_weapon_types <= MAX_WEAPON_TYPES );
	Assert( fread( Weapon_info, sizeof(weapon_info), MAX_WEAPON_TYPES, fp ) );

	Assert( fread( &N_powerup_types, sizeof(int), 1, fp ) );
	Assert( N_powerup_types > 0 && N_powerup_types <= ELMS(Powerup_info) );
	Assert( fread( Powerup_info, sizeof(powerup_type_info), MAX_POWERUP_TYPES, fp ) );
	
	Assert( fread( &N_polygon_models, sizeof(int), 1, fp ) );
	Assert( N_polygon_models > 0 && N_polygon_models <= MAX_POLYGON_MODELS );
	#if 0
	int polysize = 0;
	#endif
	for (i = 0; i <N_polygon_models; i++) {
		Assert( fread( &Polygon_models[i], sizeof(polymodel) - sizeof(ubyte *), 1, fp ) );
		#if 0
		polysize += sizeof(polymodel) - sizeof(ubyte *);
		#endif
	}
	...
	for (i=0; i<N_polygon_models; i++ )	{
		...
		Assert( fread( Polygon_models[i].model_data, sizeof(ubyte), Polygon_models[i].model_data_size, fp ) );
		#if 0
		polysize += Polygon_models[i].model_data_size;
		#endif
	}

	Assert( fread( Gauges, sizeof(bitmap_index), MAX_GAUGE_BMS, fp ) );

	Assert( fread( Dying_modelnums, sizeof(int), MAX_POLYGON_MODELS, fp ) );
	Assert( fread( Dead_modelnums, sizeof(int), MAX_POLYGON_MODELS, fp ) );

	Assert( fread( ObjBitmaps, sizeof(bitmap_index), MAX_OBJ_BITMAPS, fp ) );
	Assert( fread( ObjBitmapPtrs, sizeof(ushort), MAX_OBJ_BITMAPS, fp ) );

	Assert( fread( &only_player_ship, sizeof(player_ship), 1, fp ) );

	Assert( fread( &Num_cockpits, sizeof(int), 1, fp ) );
	Assert( Num_cockpits <= ELMS(cockpit_bitmap) );
	Assert( fread( cockpit_bitmap, sizeof(bitmap_index), N_COCKPIT_BITMAPS, fp ) );

	Assert( fread( Sounds, sizeof(ubyte), MAX_SOUNDS, fp ) );
	Assert( fread( AltSounds, sizeof(ubyte), MAX_SOUNDS, fp ) );

	Assert( fread( &Num_total_object_types, sizeof(int), 1, fp ) );
	Assert( fread( ObjType, sizeof(byte), MAX_OBJTYPE, fp ) );
	Assert( fread( ObjId, sizeof(byte), MAX_OBJTYPE, fp ) );
	Assert( fread( ObjStrength, sizeof(fix), MAX_OBJTYPE, fp ) );

	Assert( fread( &First_multi_bitmap_num, sizeof(int), 1, fp ) );

	Assert( fread( &N_controlcen_guns, sizeof(int), 1, fp ) );
	Assert( N_controlcen_guns > 0 && N_controlcen_guns <= ELMS(controlcen_gun_points) );
	Assert( fread( controlcen_gun_points, sizeof(vms_vector), MAX_CONTROLCEN_GUNS, fp ) );
	Assert( fread( controlcen_gun_dirs, sizeof(vms_vector), MAX_CONTROLCEN_GUNS, fp ) );
	Assert( fread( &exit_modelnum, sizeof(int), 1, fp ) );
	Assert( fread( &destroyed_exit_modelnum, sizeof(int), 1, fp ) );
	//printf("%d %d %d\n", cftell(fp) - polysize, polysize, N_polygon_models);
	//exit(1);
}
